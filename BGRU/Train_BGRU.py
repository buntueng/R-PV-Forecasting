...
class BGRUModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim):
        ...
        self.hidden_dim = hidden_dim
        self.num_layers = num_layers
        self.bgru = nn.GRU(input_dim, hidden_dim, num_layers, batch_first=True, bidirectional=True)
        self.fc = nn.Linear(hidden_dim * 2, output_dim)  # Multiply by 2 for bidirectional
        
    def forward(self, x):
        h0 = torch.zeros(self.num_layers * 2, x.size(0), self.hidden_dim).to(x.device)
        output, hn = self.bgru(x, h0)
        output = output[:, -1, :]
        output = self.fc(output)
        return output
...
if __name__ == "__main__":
    ...
    model = BGRUModel(input_dimension, hidden_dimension, number_of_layers, output_dimension).to(device)
