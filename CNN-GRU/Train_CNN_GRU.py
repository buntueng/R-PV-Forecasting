...
# CNN-BLSTM Model definition
class CNN_BLSTMModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim):
        super(CNN_BLSTMModel, self).__init__()
        ...
        self.fc1 = nn.Linear(input_dim, input_dim)  # Adjust output dimension as needed

        # Bidirectional LSTM layer (replacing LSTM)
        self.blstm = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True, bidirectional=True)

        # Fully connected layer after BLSTM (adjust input dimension)
        self.fc2 = nn.Linear(hidden_dim * 2, output_dim)  # Multiply hidden_dim by 2 for bidirectional

    def forward(self, x):
        # CNN and ReLU
        x = x.permute(0, 2, 1)
        x = self.cnn(x)
        x = torch.relu(x)  # Apply ReLU after CNN
        x = x.permute(0, 2, 1)

        # Fully connected and ReLU
        x = self.fc1(x)
        x = torch.relu(x)  # Apply ReLU after fully connected

        # BLSTM
        h0 = torch.zeros(self.num_layers * 2, x.size(0), self.hidden_dim).to(x.device)  # Multiply num_layers by 2 for bidirectional
        c0 = torch.zeros(self.num_layers * 2, x.size(0), self.hidden_dim).to(x.device)  # Multiply num_layers by 2 for bidirectional
        output, (hn, cn) = self.blstm(x, (h0, c0))  # BLSTM returns output, hidden state, and cell state

        # Fully connected after BLSTM
        output = output[:, -1,:]
...
if __name__ == "__main__":
    ...
    model = CNN_BLSTMModel(input_dimension, hidden_dimension, number_of_layers, output_dimension).to(device)
