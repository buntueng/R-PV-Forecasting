...
class CNN_GRUModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # GRU
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_dim).to(x.device)
        output, hn = self.gru(x, h0)  # GRU returns output and hidden state

        # Fully connected after GRU
        output = output[:, -1,:]
...
if __name__ == "__main__":
    ...
    model = CNN_GRUModel(input_dimension, hidden_dimension, number_of_layers, output_dimension).to(device)
